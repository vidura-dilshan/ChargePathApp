...
class _BookStationState extends State<BookStation> {
  ...
  void initState() {
    ...
    if (widget.preSelectedStationId != null) {
      _selectedStationId =
          widget.preSelectedStationId;

      _selectedStationName =
          widget.preSelectedStationName;
  ...
  Future<void> _loadAllStations() async {
    try {
      ...
      setState(() {
        ...
        // If we only received the station ID,
        // retrieve its data from the loaded list.
        if (_selectedStationId != null &&
            _selectedStationData == null) {
  ...
  // ---------------------------------------------------------------------------
  // FIRESTORE BOOKING DATE / TIME FORMAT
  // ---------------------------------------------------------------------------
  ...
  Future<void> _confirmBooking() async {
    ...
    if (currentUser == null) {
      ScaffoldMessenger.of(context)
        ...
        const SnackBar(
          content: Text(
            'You must be signed in to book a station.',
          ),
        ),
      );
    ...
    try {
      ...
      // Use a transaction so two drivers cannot
      // silently overwrite each other's booking.
      await FirebaseFirestore.instance
          .runTransaction(
            (
          ...
          if (!stationSnapshot.exists) {
            throw Exception(
              'This charging station no longer exists.',
            );
          }
          ...
          final String existingBookingUserId =
              stationData[
              'booking_user_id']
                  ?.toString()
                  .trim() ??
          ...
          // Prevent overwriting another driver's booking.
          if (existingBookingUserId
              .isNotEmpty &&
              existingBookingUserId !=
                  currentUser.uid) {
            throw Exception(
              'This station already has an active booking.',
            );
          }
      ...
      // after Firestore successfully saves it.
      ...
      if (e.code ==
          'permission-denied') {
        message =
        'Booking permission was denied by Firestore.';
      }
                    ...
                    children: [
                      ...
                      _popupDetailRow(
                        Icons
                            .payments_outlined,
                        'Charging Rate',
                        'Rs.${costPerKw.toStringAsFixed(2)} / kW',
                      ),
                    ],
                  ),
                ),
          ...
          if (_selectedStationData !=
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Rs.${_getCostPerKw().toStringAsFixed(2)} / kW',
                    style:
                    const TextStyle(
                      color:
                      AppColors
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'This is the station charging rate. '
                        'The final amount paid will be recorded '
                        'after the charging session.',
                    style:
                    const TextStyle(
                      color:
                      AppColors.primary,
  ...
  Widget _buildBalanceNotice() {
    return Center(
      child: Text(
        'Final amount will be recorded after charging.',
        style:
        const TextStyle(
          color:
